...
DE_Genevestigator <- function(data, AMP_data, PRO_data, AMP = 2, PRO = 10){
  ...
  return(data[data$Compounds %in% compounds_to_be_stored,])
}
```
...
finding_selected_compounds <-function(results_data){
  selected_words <- c()
  for (i in 1:length(results_data$Compounds)){
    selected_words[i] <- str_extract(results_data$Compounds[i], "([^\\s]+)") } # extract the first word = compound
...
LINC_L1000_modified <-cbind(as.data.frame(LINC_compound),as.data.frame(LINC_time),as.data.frame(LINC_conc),as.data.frame(LINC_cell),as.data.frame(LINC_modified_perturbations),LINC_L1000_modified)

colnames(LINC_L1000_modified) <- c("Compounds", "Time","Concentration", "Cell", "Original","BM2", "DEFB1","DEFB4A", "CAMP", "REG3A", "PLA2G2A", "IL1B", "IL8", "TNF")
```
...
for (i in 1:nrow(results_LINC)){
  LINC_100_ratio_values[i] <- (sum(results_LINC[i,AMP_LINC_L1000],na.rm = TRUE)/length(AMP_LINC_L1000))/(sum(results_LINC[i,PRO_LINC_L1000],na.rm = TRUE)/length(PRO_LINC_L1000))
}
